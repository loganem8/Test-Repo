function [] = PRINT_EXCEL_SUBPPT_TRANS(Title_initial,Time_Date_Full,Data_Accel,Data_Gyro,Data_Accel_Cal,Data_Gyro_Cal,Data_Accel_Cal_Filt_Zero,Data_Gyro_Cal_Filt_Zero,Data_Accel_Trans,Data_Gyro_Trans,Linear_Resultant_CG,Rotational_Resultant_CG,Files_Names,Impact_Totals,Filter)
...
%% Excel Files
    ...
    Data_Transformed_Pre = [Data_Accel_Trans.Impact Data_Accel_Trans.Index round(Data_Accel_Trans.AccelX,2) round(Data_Accel_Trans.AccelY,2) round(Data_Accel_Trans.AccelZ,2) round(Data_Gyro_Trans.GyroX,2) round(Data_Gyro_Trans.GyroY,2) round(Data_Gyro_Trans.GyroZ,2) round(Data_Accel_Trans.Timestamp,2)];
    Data_Transformed = array2table(Data_Transformed_Pre, 'VariableNames', {'Impact' 'Index' 'AccelX' 'AccelY' 'AccelZ' 'GyroX' 'GyroY' 'GyroZ' 'Timestamp'});
    ...
    Filename_Transformed_cha = sprintf('%s_Transformed.xlsx', Title_initial);
    ...
    writetable(Data_Transformed,Filename_Transformed_cha,'Sheet',1);
        writetable(Event_Times,Filename_Transformed_cha,'Sheet',2);
...
imshow('Legend.jpg')
    for p=1:length(Impact_unique)
        ...
        Title = sprintf('%s (%s, %s)',MP,Time_Title,Index);
            ...
            figure
                subplot(2,2,1)
                    plot(Data_Accel_Trans.Timestamp((p*282-281):(p*282)),Data_Accel_Trans.AccelX((p*282-281):(p*282)),'Color',[0, 0.4470, 0.7410])
                    hold on
                    plot(Data_Accel_Trans.Timestamp((p*282-281):(p*282)),Data_Accel_Trans.AccelY((p*282-281):(p*282)),'Color',[0.8500, 0.3250, 0.0980])
                    plot(Data_Accel_Trans.Timestamp((p*282-281):(p*282)),Data_Accel_Trans.AccelZ((p*282-281):(p*282)),'Color',[0.9290, 0.6940, 0.1250])
                    hold off
...
%                     legend('X axis','Y axis','Z axis','Location','southeast')
                    ...
                    xticks(-15:5:45)
                subplot(2,2,3)
                    plot(Data_Accel_Trans.Timestamp((p*282-281):(p*282)),Linear_Resultant_CG((p*282-281):(p*282)),'Color','k')
                    ...
                    xticks(-15:5:45)
                subplot(2,2,2)
                    plot(Data_Gyro_Trans.Timestamp((p*282-281):(p*282)),Data_Gyro_Trans.GyroX((p*282-281):(p*282)),'Color',[0, 0.4470, 0.7410])
                    hold on
                    plot(Data_Gyro_Trans.Timestamp((p*282-281):(p*282)),Data_Gyro_Trans.GyroY((p*282-281):(p*282)),'Color',[0.8500, 0.3250, 0.0980])
                    plot(Data_Gyro_Trans.Timestamp((p*282-281):(p*282)),Data_Gyro_Trans.GyroZ((p*282-281):(p*282)),'Color',[0.9290, 0.6940, 0.1250])
                    hold off
...
%                     legend('X axis','Y axis','Z axis','Location','southeast')
                    ...
                    grid minor
                subplot(2,2,4)
                    plot(Data_Gyro_Trans.Timestamp((p*282-281):(p*282)),Rotational_Resultant_CG((p*282-281):(p*282)),'Color','k')
